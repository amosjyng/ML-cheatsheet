...
Given a region, $\hat{p}(x)$ majority class, where $\hat{p}$ is the probability of getting it correct. Error rate is $1 - \hat{p}(x)$. As the \# of neighbors go to $\infty$ as $n \rightarrow \infty$, $k \in \ensuremath{\mathcal{O}}(\sqrt{n})$
...
Minimize $c^T x$ subject to $A x \leq b$ and $x \geq 0$.

\subsection{Quadratic Programming}

Minimize $f(x) = \frac{1}{2} x^T Q x + c^T x$ subject to either
\begin{itemize}
	\item $A x \leq b$ (inequality constraint)
	\item $E x = d$ (equality constraint)
\end{itemize}
...
Rohan:
\begin{align*}
B_j &:= B_j - \alpha \frac{\delta}{\delta \beta_j} J(\beta)\\
...
P(y_i | x_i, \theta) &= \frac{1}{\sqrt{2 \pi} \sigma} exp \left( - \frac{(y_i - \theta^T x_i)^2}{2 \gamma^2} \right)
\end{align*}
