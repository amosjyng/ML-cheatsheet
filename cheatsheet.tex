...
\end{equation*}
Converges quadratically in the region near the optimal value, given that the Hessian can be found and that $f(x)$ is twice differentiable.

BFGS approximates Newton's method.
...
\begin{itemize}
	\item \emph{Primal constraint satisfaction:} $f_i (x) \leq 0,\:i = 1, ..., m;\:h_i(x) = 0,\:i = 1, ..., p$
	\item \emph{Dual constrain satisfaction:} $\lambda \geq 0$
